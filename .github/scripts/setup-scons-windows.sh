...
py -3 -m venv "$RUNNER_TEMP/scons"
"$RUNNER_TEMP/scons/Scripts/python.exe" -m pip install --no-deps "$wheel"
"$RUNNER_TEMP/scons/Scripts/scons.exe" --version | grep -F "SCons: v4.10.1"

if command -v cygpath >/dev/null 2>&1; then
	cygpath -u "$RUNNER_TEMP/scons/Scripts" >>"$GITHUB_PATH"
else
	printf '%s\n' "$RUNNER_TEMP/scons/Scripts" >>"$GITHUB_PATH"
fi
